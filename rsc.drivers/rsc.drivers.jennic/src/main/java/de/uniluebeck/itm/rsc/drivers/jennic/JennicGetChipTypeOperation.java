...
public class JennicGetChipTypeOperation extends AbstractOperation<ChipType> implements GetChipTypeOperation {
	...
	private ChipType getChipType(final AbstractProgressManager monitor) throws Exception {
		...
		try {
			// Read chip type read response
				...
				if (res.length == 6){
					...
					else log.error("BYTES incorrect res[1]="+res[1]+" != "+0+ "res[2]="+res[2]+" != "+0x10+ "res[3]="+res[3]+" != "+0x40+ "res[4]="+res[4]+" != "+0x46+ "res[5]="+res[5]+" != "+(byte)0x86);
				} else log.error("length incorrect");
			} 
