...
public class SunspotBaseStation {
	...
	private static class OperationQueueEntry {
		...
		public OperationListener callback;

		private OperationQueueEntry(Operation operation, OperationFuture future,
	...
	public void start() {
		if (this.ftime) {
			...
			System.setProperty("squawk.startup.arguments",
					"-Xboot:268763136 -Xmxnvm:0 -isolateinit:com.sun.spot.peripheral.Spot -dma:1024"
			);

			opExecutor = new Thread(operationExecutor);
			opExecutor.start();

			msgBasestation = new Thread(
					new SunspotMessageBasestation(this.ReceivingRadiogramPort, this.ControlRadiogramPort,
							this.listeners, sendMessageQueue
					)
			);
	...
	public void stop() {
		opExecutor.interrupt();
		msgBasestation.interrupt();

	}


	public OperationFuture<Void> resetNode(String macAddress, long timeout, OperationListener<Void> callback) {
		SunspotResetOperation operation = new SunspotResetOperation(
				macAddress, this.sysBinPath, this.libFilePath, this.keyStrorePath, this.port, this.iport
		);

		Operation<Void> operationContainer = factory.create(operation, timeout, callback);
		OperationFutureTask<Void> future = new OperationFutureTask<Void>(operationContainer);
		this.operationQueue.add(new OperationQueueEntry(operation, future, callback));
		return future;
	}

	public OperationFutureTask<Void> isNodeAlive(String macAddress, long timeout, OperationListener<Void> callback) {
		SunspotIsAliveOperationRunnable operationRunnable =
				new SunspotIsAliveOperationRunnable(macAddress, this.sysBinPath, this.libFilePath, this.keyStrorePath,
						this.port, this.iport
				);
	...
	public OperationFutureTask<Void> program(String macAddress, byte[] jar, long timeout,
											 OperationListener<Void> callback) throws Exception {
		SunspotProgramOperation operation =
				new SunspotProgramOperation(macAddress, this.sysBinPath, this.libFilePath, this.keyStrorePath,
						this.port, this.iport, jar, this.workingDirectory
				);
		Operation<Void> operationContainer = factory.create(operation, timeout, callback);
		OperationFutureTask<Void> future = new OperationFutureTask<Void>(operationContainer);
		this.operationQueue.add(new OperationQueueEntry(operation, future, callback));
	...
	public OperationFuture<ChipType> getChipType(String macAddress, long timeout,
												 OperationListener<ChipType> callback) {
	...
	public OperationFutureTask<Void> send(String macAddress, byte[] message, long timeout,
										  OperationListener<Void> callback) {
		SunspotSendOperation operation =
				new SunspotSendOperation(macAddress, message, this.ControlRadiogramPort);
		Operation<Void> operationContainer = factory.create(operation, timeout, callback);
		OperationFutureTask<Void> future = new OperationFutureTask<Void>(operationContainer);
		this.operationQueue.add(new OperationQueueEntry(operation, future, callback));
		return future;
