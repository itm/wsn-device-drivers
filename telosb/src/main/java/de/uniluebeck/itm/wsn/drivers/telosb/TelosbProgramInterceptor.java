...
public class TelosbProgramInterceptor extends SerialPortProgramInterceptor {

	/**
	 * Logger for this class.
	 */
	private static final Logger LOG = LoggerFactory.getLogger(TelosbProgramInterceptor.class);
	
	private final Provider<BSLTelosb> bslProvider;
	
	public TelosbProgramInterceptor(SerialPortConnection connection, Provider<BSLTelosb> bslProvider) {
		super(connection);
		this.bslProvider = bslProvider;
	}

	@Override
	public void enterProgramMode(SerialPortConnection connection) throws Exception {
		BSLTelosb bsl = bslProvider.get();
	...
	private int readBSLVersion(BSLTelosb bsl) throws Exception {
		...
		LOG.trace(String.format("Current bslProvider version: %02x.%02x, device id: 0x%04x", ((bslVersion >> 8) & 0xFF), (bslVersion & 0xFF), deviceId));
		return bslVersion;
	}
}
