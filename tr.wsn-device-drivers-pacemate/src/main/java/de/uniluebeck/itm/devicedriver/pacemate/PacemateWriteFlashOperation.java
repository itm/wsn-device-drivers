...
public class PacemateWriteFlashOperation extends AbstractWriteFlashOperation {
	...
	private void write(Monitor monitor) throws Exception {
		...
		final PacemateBinData binData = new PacemateBinData(getAddress(), getData());
		...
		try {
			...
			log.error("Error while write CRC to Flash! Operation will be cancelled!", e);
			throw e;
		}
			...
			while (counter < block.data.length) {
				...
				if (counter + 45 < block.data.length) {
					line = new byte[PacemateBinData.LINESIZE]; // a line with 45
																// bytes
					System.arraycopy(block.data, counter, line, 0,
							PacemateBinData.LINESIZE);
					counter = counter + PacemateBinData.LINESIZE;
					bytesNotYetProoved = bytesNotYetProoved
							+ PacemateBinData.LINESIZE;
				} else {
