	...
	private void processIncomingBinaryFrame() {
		try {
			...
			case RECEIVE_PACKET_802154:
				...
				DoubleByte cmdTypeAndReqID = new DoubleByte(xbeeResponse.getPayload()[0], xbeeResponse.getPayload()[1]);
				XBeeProtocol protocol = frameType.protocol();
				switch (protocol) {
				case PROTOCOL_DIGIMESH:
					if (cmdTypeAndReqID.equals(new DoubleByte(SmartSantanderFrame.SERVICE_FRAME_HEADER))) {
						...
						// Substitute nodeID with cmdTypeAndReqID if needed
						//(see write function above)
					}
					break;
